...
<!-- GENERATED by tests/compose-context.py from context/manifest.json.
     Skill: end-session
     Fragments: 00-head-skill, 01-intro, 02-action-tiers, 03-pre-flight, 04-surface-sandbox, 05-phase-1, 06-gather-sandbox, 07-fast-path-sandbox, 08-fetch-prune, 09-ci-on-demand, 10-uncommitted-work, 11-return-to-main, 12-prune-branches-sandbox, 13-push-main, 13a-config-reach, 14-open-prs-sandbox, 15-stashes, 16-assigned-issues-sandbox, 17-stale-claude-files-sandbox, 18-worktrees-sandbox, 19-background-processes, 19a-gcp-projects, 19b-armed-triggers, 20-summary-sandbox, 21-phase-2, 22-guardrails-sandbox

...
<!-- GENERATED by tests/compose-context.py from context/manifest.json.
     Skill: start-session
     Fragments: 00-head-skill, 01-intro, 02-action-tiers, 03-surface-sandbox, 03b-archived-repo, 04-phase-1, 05-gather-sandbox, 06-fetch-result, 07-sync-default-branch, 08-ci-on-demand, 09-recent-merges, 10-config-drift-sandbox, 11-journal-inbox, 11a-ready-bundles, 12-session-brief-sandbox, 13-guardrails

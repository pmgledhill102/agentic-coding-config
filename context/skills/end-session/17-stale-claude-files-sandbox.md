...
`cloud/bootstrap.sh` writes its own files from an explicit list, the platform
launcher rewrites its own each session, and the container is discarded rather
than updated in place. Nothing accumulates.

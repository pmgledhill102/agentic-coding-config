...
| `repo_archived` | 3b, 7 | Always present, never absent. `state=true` / `false` / `unknown`. On this surface it reads `unknown` every time, because the flag rides the same repo-scoped probe the proxy 403s — report it as unchecked, never as "not archived". |

...
- Common mappings: PRs (`create_pull_request`, `pull_request_read`, `update_pull_request`, `merge_pull_request`, `list_pull_requests`, `search_pull_requests`), reviews (`pull_request_review_write`, `add_comment_to_pending_review`, `add_reply_to_pull_request_comment`), issues (`issue_read`, `issue_write`, `list_issues`, `search_issues`, `add_issue_comment`), releases (`get_latest_release`, `list_releases`, `get_release_by_tag`), repo content (`get_file_contents`, `list_commits`, `get_commit`, `list_branches`, `create_branch`), search (`search_code`, `search_repositories`). **Two of these overflow the context limit unless called carefully.** `list_issues` must always be given `fields` — `["number", "title", "labels", "assignees", "state"]` costs around 750 tokens for a 36-issue repo, where an unfiltered call blows the limit somewhere past roughly 25 issues; never add `body`. `list_pull_requests` has **no field filter at all**, so it cannot be trimmed the same way: keep `perPage` small and pull detail for a single PR with `pull_request_read`. Their `state` values also differ in case — `list_issues` takes `"OPEN"`, `list_pull_requests` takes `"open"`
